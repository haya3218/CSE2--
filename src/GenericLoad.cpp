...
BOOL LoadGenericData(void)
{
	...
	if (!MakeSurface_File("Resource/BITMAP/Credit01", SURFACE_ID_CREDITS_IMAGE))
		bError = TRUE;
	if (!MakeSurface_File("optionsBG", SURFACE_ID_MENU_OVERLAY))
		bError = TRUE;
	if (!MakeSurface_File("box", SURFACE_ID_TITLEM))
		bError = TRUE;
	if (!MakeSurface_File("optionsBox", SURFACE_ID_OPTIOM))
		bError = TRUE;
	if (!MakeSurface_File("saveBox", SURFACE_ID_SAVE_BOX))
		bError = TRUE;
	if (!MakeSurface_File("saveBoxx", SURFACE_ID_BEEG_SAVE_BOX))
		bError = TRUE;
	if (!MakeSurface_File("minus", SURFACE_ID_MINUS))
		bError = TRUE;
	if (!MakeSurface_File("saveCur", SURFACE_ID_SAVE_CURSOR))
		bError = TRUE;
	...
	MakeSurface_Generic(320, 240, SURFACE_ID_ROOM_NAME, FALSE, TRUE);
