...
#define IS_COMMAND_FOUR(c1, c2, c3, c4) (gTS.data[gTS.p_read + 1] == (c1) && gTS.data[gTS.p_read + 2] == (c2) && gTS.data[gTS.p_read + 3] == (c3) gTS.data[gTS.p_read + 4] == (c4))
...
// Get 12 digit number from TSC data
int GetTextScriptNoAlt(int a)
{
	int b = 0;
	b += (gTS.data[a++] - '0') * 100000000000;
	b += (gTS.data[a++] - '0') * 10000000000;
	b += (gTS.data[a++] - '0') * 1000000000;
	b += (gTS.data[a++] - '0') * 100000000;
	b += (gTS.data[a++] - '0') * 10000000;
	b += (gTS.data[a++] - '0') * 1000000;
	b += (gTS.data[a++] - '0') * 100000;
	b += (gTS.data[a++] - '0') * 10000;
	b += (gTS.data[a++] - '0') * 1000;
	b += (gTS.data[a++] - '0') * 100;
	b += (gTS.data[a++] - '0') * 10;
	b += gTS.data[a] - '0';
...
int TextScriptProc(void)
{
	...
	switch (gTS.mode)
	{
		case 1: // PARSE
			...
			while (!bExit)
			{
				if (gTS.data[gTS.p_read] == '<')
				{
					...
					{
						saving = true;
						int ret = Call_SaveMenu();
						if (ret == 1337) {
							SaveProfile(*"Prof" + std::to_string(sCounter + 1).c_str());
							gTS.p_read += 4;
							saving = false;
						} else if (ret == 6) {
							gTS.mode = 0;
					...
					{
						if (!LoadProfile(*"Prof" + std::to_string(sCounter + 1).c_str()))
							InitializeGame();
					}
					...
					// <EQC from equip+, without the reload shit
					// why? because im too lazy to do that right now
					// and that would remove the purpose of uberpowered weapons anyway
					// you idiot
					else if (IS_COMMAND('E','Q','C'))
					{
						x = GetTextScriptNo(gTS.p_read + 4);
						y = GetTextScriptNo(gTS.p_read + 9);
						...
						switch (x) {
							case 40:
								gMC.health_threshold = y;
								break;
							case 41:
								gMC.alt_health_threshold = y;
								break;
							case 42:
								gMC.health_regen = y;
								break;
							case 43:
								gMC.health_regen_am = y;
								break;
							case 44:
								gMC.alt_health_regen = y;
								break;
							case 45:
								gMC.alt_health_regen_am = y;
								break;
							case 52 | 53 | 54 | 55:
								// too lazy to make it separate
								gMC.boost_cnt = y;
								break;
							case 56 | 57:
								gCurrentFPS = y;
							case 58 | 59: // timescale >:) dont set it too high or everything will fall
								gCurrentTimeScale = y;
						}

						gTS.p_read += 13;
					}
					// <MODwwww:0123456789AB
					// 0 - ignore (will always return true)
					// 1 - required to be true
					// 2 - required to be false
					// 3 - disable (will always return false)
					// work in progress
					else if (IS_COMMAND('M','O','D'))
					{
						x = GetTextScriptNo(gTS.p_read + 4);
						int num_array[13];
						for (int i = 0; i < 13; i++) {
							num_array[i] = GetTextScriptNoIn(gTS.p_read + (9 + i));
					...
					// <DOR, its like TRA but less shite
					// <DORzzzz:wwww:xxxx:yyyy:ssss:gggg
					// z is map num, w is event num, x and y are explanatory, s is music num
					// and g is npc num to remove. if its set to zero then the g value is ignored
					else if (IS_COMMAND('D','O','R'))
					{
						int g = GetTextScriptNo(gTS.p_read + 29);
						if (g != 0)
							DeleteNpCharEvent(g);

						// start fade out and play door sound effect if g is not 0
						if (g != 0)
						...
						gMC.shock = 0;
						StartFadeOut(0);
						// check mode 8 for the fade ins and player transports
						gTS.mode = 8;
					}
					else
					{
		...
		case 8: // DOR
			...
			x = GetTextScriptNo(gTS.p_read + 14);
			y = GetTextScriptNo(gTS.p_read + 19);
			int s = GetTextScriptNo(gTS.p_read + 24);
			...
			// change music and start fading in
			ChangeMusic((MusicID)s);
			bExit = TRUE;
			bLoad = false;
			StartFadeIn(0);
			gMC.cond &= ~1;
