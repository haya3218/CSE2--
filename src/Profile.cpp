...
const char* gDefaultName = "Prof1.dat";
...
{
	std::string stringName("Prof" + std::to_string(sCounter + 1) + ".dat");
	const char* charName;
	charName = &stringName[0];

	std::string path = gModulePath + '/' + charName;
...
BOOL SaveProfile(const char *name)
{
	...
	File_WriteLE32(profile.health_threshold, fp);
	File_WriteLE32(profile.alt_health_threshold, fp);
	File_WriteLE32(profile.health_regen, fp);
	File_WriteLE32(profile.alt_health_regen, fp);
	File_WriteLE32(profile.health_regen_am, fp);
	File_WriteLE32(profile.alt_health_regen_am, fp);
...
BOOL LoadProfile(const char *name)
{
	...
	// i didnt have to do this shit fuck ass
	std::string stringName("Prof" + std::to_string(sCounter + 1) + ".dat");
	const char* charName;
	charName = &stringName[0];
	...
	if (memcmp(profile.extra_code, gProfileCodeExtra, 0x10) == 0)
	{
		...
		profile.health_threshold = File_ReadLE32(fp);
		profile.alt_health_threshold = File_ReadLE32(fp);
		profile.health_regen = File_ReadLE32(fp);
		profile.alt_health_regen = File_ReadLE32(fp);
		profile.health_regen_am = File_ReadLE32(fp);
		profile.alt_health_regen_am = File_ReadLE32(fp);
	}
	...
	if (memcmp(profile.extra_code, gProfileCodeExtra, 0x10) == 0)
	{
		...
		gMC.health_threshold = profile.health_threshold;
		gMC.alt_health_threshold = profile.alt_health_threshold;
		gMC.health_regen = profile.health_regen;
		gMC.alt_health_regen = profile.alt_health_regen;
		gMC.health_regen_am = profile.health_regen_am;
		gMC.alt_health_regen_am = profile.alt_health_regen_am;
	}
...
PROFILEDATA LoadProfileSimple(int num)
{
	...
	// i didnt have to do this shit fuck ass
	std::string stringName("Prof" + std::to_string(num) + ".dat");
	const char* charName;
	charName = &stringName[0];

	path = gModulePath + '/' + charName;
