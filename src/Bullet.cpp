...
//			PutBitmap3(&grcGame, SubpixelToScreenCoord(x) - SubpixelToScreenCoord(fx)- 16, SubpixelToScreenCoord(y) - SubpixelToScreenCoord(fy) - 16, &lightRect, SURFACE_ID_LIGHT);
...
BULLET_TABLE gBulTbl[] = {
	...
	// better version of explos
	{100, 100, 100, 20, 80, 80, 0, 0, {0, 0, 0, 0}},
	// barrel projectile
	{0, 10, 30, 40, 2, 2, 2, 2, {8, 8, 8, 8}},
};
void SetBulletExt(int no, int x, int y, int xm, int ym, int lifetime, int direct){
    int i = 0;
...
void ActBullet_HELL(BULLET *bul, int level)
{
	BOOL bHit;
	...
	if (bul->direct == DIR_RIGHT && bul->flag & 4)
		bHit = TRUE;
	if ((bul->direct == DIR_UP || bul->direct == DIR_LEFT || bul->direct == DIR_RIGHT) && bul->flag & 2)
		bHit = TRUE;
	if ((bul->direct == DIR_DOWN || bul->direct == DIR_LEFT || bul->direct == DIR_RIGHT) && bul->flag & 8)
		bHit = TRUE;
	if (bul->direct == DIR_LEFT && bul->flag & 0x80)
		bHit = TRUE;
	if (bul->direct == DIR_LEFT && bul->flag & 0x20)
	...
	if (bHit)
	{
		SetBullet(46, bul->x, bul->y, DIR_LEFT);
		bul->cond = 0;
	}
...
void ActBullet_HolyBom(BULLET *bul)
{
	switch (bul->act_no)
	{
		case 0:
			...
			SetNpChar(362, bul->x, bul->y, 0, 0, 2, NULL, 0);

			PlaySoundObject(44, SOUND_MODE_PLAY);
			// Fallthrough
		case 1:
			for (int i = 0; i < 5; i++) {
				SetDestroyNpCharUp(bul->x, bul->y, bul->enemyXL, 10);
			}
...
//		SetCaret(bul->x, bul->y, CARET_SHOOT, DIR_LEFT);
	...
	{
		bul->count2 = 0;
        int particle = CARET_TINY_PARTICLES;
		switch (bul->direct)
		{
			case DIR_LEFT:
				SetCaret(bul->x + (8 * 0x200), bul->y, particle, DIR_RIGHT);
				break;
			case DIR_UP:
				SetCaret(bul->x, bul->y + (8 * 0x200), particle, DIR_DOWN);
				break;
			case DIR_RIGHT:
				SetCaret(bul->x - (8 * 0x200), bul->y, particle, DIR_LEFT);
				break;
			case DIR_DOWN:
				SetCaret(bul->x, bul->y - (8 * 0x200), particle, DIR_UP);
				break;
		}
	}
	...
	for (i = 0; i < BULLET_MAX; ++i)
	{
		if (gBul[i].cond & 0x80)
		{
			...
			switch (gBul[i].code_bullet)
			{
				...
				case 46:
					ActBullet_HolyBom(&gBul[i]);
					break;

				case 47:
					ActBullet_HELL(&gBul[i], 3);
					break;
			}
		}
	}
