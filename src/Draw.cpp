...
BOOL gbFR;
BOOL gbNoDamage;
unsigned int gCurrentCion = 0;
int gCurrentFPS = -1;
int gCurrentTimeScale = 1;
...
BOOL Flip_SystemTask(void)
{
	...
	else
	{
		...
		unsigned int delay = gb60fps ? frameDelays[frame % 3] : 20;
		if(gCurrentFPS != -1){
...
void InitTextObject(const char *name)
{
	...
	bool antialiasing = false;	// The 1x font looks better without antialiasing
#endif

#ifdef FREETYPE_FONTS
	std::string path = gDataPath + "/Font/font.ttf";
...
#ifdef FREETYPE_FONTS
void SetAltFont(const char *font_name) {
	bool antialiasing;

	font_height = 12;
	font_width = 12;

	antialiasing = mag < 3;	// The 1x font looks better without antialiasing

	std::string path = gDataPath + "/Font/" + font_name + ".ttf";

	alt_font = LoadFreeTypeFont(path.c_str(), font_width * mag, font_height * mag, antialiasing);
}
#endif

void PutText(int x, int y, const char *text, unsigned long color, bool alt_font_enabled)
{
	Font *temp_font;
	if (alt_font_enabled)
		temp_font = alt_font;
	...
	if (gbFR)
		temp_font = alt_font;
	DrawText(temp_font, framebuffer, x * mag, y * mag, color, text);
}
...
void PutText2(int x, int y, const char *text, unsigned long color, SurfaceID surf_no)
{
	...
	DrawText(temp_font, surf[surf_no], x * mag, y * mag, color, text);
}
...
std::string imageArray[SURFACE_ID_MAX - 45];

void DrawImageFromFile(std::string path, const RECT *rcView, int x, int y, const RECT *rect) {
	RECT rcRect = {0, 0, 320, 340};
	if (rect == NULL) {
		rect = &rcRect;
	}
	for (int i = 0; i < (SURFACE_ID_MAX - 45); i++) {
		// if it already exists in the image array, just render it as normal.
		if (imageArray[i] == path) {
			PutBitmap3(rcView, x, y, rect, (SurfaceID)(i + 45));
			return;
		}
	}
	...
	for (int i = 0; i < (SURFACE_ID_MAX - 45); i++) {
		...
		if (imageArray[i] == path) {
			MakeSurface_File(path.c_str(), (SurfaceID)(i + 45));
			PutBitmap3(rcView, x, y, rect, (SurfaceID)(i + 45));
		}
	}
}
