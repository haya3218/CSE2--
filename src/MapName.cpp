...
void PutTextCentred(int x, int y, const char *text)
{
	...
	PutText2(x - string_width / 1.5, y, text, RGB(0x11, 0x00, 0x22), SURFACE_ID_ROOM_NAME);
	PutText2(x - string_width / 1.5, y - 1, text, RGB(0xFF, 0xFF, 0xFE), SURFACE_ID_ROOM_NAME);
}
...
void ReadyMapName(const char *str)
{
	...
	PutText2(((160 - (a * 6)) / 2) + 12, 1, gMapName.name, RGB(0x11, 0x00, 0x22), SURFACE_ID_ROOM_NAME);
	PutText2(((160 - (a * 6)) / 2) + 12, 0, gMapName.name, RGB(0xFF, 0xFF, 0xFE), SURFACE_ID_ROOM_NAME);
}
