...
BOOL InitMapData2(void)
{
	...
	gMap.farBackData = (unsigned char*)malloc(PXM_BUFFER_SIZE);
	gMap.backData = (unsigned char*)malloc(PXM_BUFFER_SIZE);
	gMap.frontData = (unsigned char*)malloc(PXM_BUFFER_SIZE);
	return TRUE;
}

void ReadLayerData(unsigned char* dest, int half_size, FILE* fp){
...
BOOL LoadMapData2(const char *path_map)
{
	...
	ReadLayerData(gMap.farBackData, gMap.width * gMap.length, fp);
	ReadLayerData(gMap.backData, gMap.width * gMap.length, fp);
	ReadLayerData(gMap.data, gMap.width * gMap.length, fp);
	ReadLayerData(gMap.frontData, gMap.width * gMap.length, fp);

	fclose(fp);
	return TRUE;
}

void PutStage_Layer(unsigned char* data, int fx, int fy){
    int i, j;
    RECT rect;
    int offset;
    ...
    for (j = put_y; j < put_y + num_y; ++j)
    {
        for (i = put_x; i < put_x + num_x; ++i)
        {
            ...
            // Remove the seperation of drawing tiles, we always draw the layer exactly when its called
            // The "vanilla" layer is seperated into 2 draw calls, 1 behind entities, 1 in front
                        /*if (atrb >= 0x20)
                            continue;*/
...
void PutStage_Back(int fx, int fy)
{
    ...
    PutStage_Layer(gMap.farBackData, fx, fy);
    PutStage_Layer(gMap.backData, fx, fy);

    // Continue with the vanilla layer
	int i, j;
	RECT rect;
	int offset;
...
void PutStage_Front(int fx, int fy)
{
    ...
    PutStage_Layer(gMap.frontData, fx, fy);
}
